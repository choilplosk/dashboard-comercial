...
from modulos.supabase_db import carregar_nps_supabase
...
def _render_tabela(base_iaf: pd.DataFrame):
    ...
    fig.update_layout(
        height=420, margin=dict(t=20, b=20, r=80),
...
def _render_configuracao():
    ...
    h1.markdown("**Nome**"); h2.markdown("**Pontos**"); h3.markdown("**Coluna realizado**")
    h4.markdown("**Coluna meta**"); h5.markdown("**Lógica**"); h6.markdown("**Ação**")
    ...
    for ind in indicadores:
        ...
        with c2:
            pontos = st.number_input("", value=float(ind['pontos']), min_value=0.0,
                                      max_value=500.0, step=5.0, key=f"iaf_pts_{ind['id']}",
                                      label_visibility='collapsed')
        with c3:
            ...
            col_real = st.selectbox("", labels_real, index=idx_real,
                                     key=f"iaf_real_{ind['id']}", label_visibility='collapsed')
        ...
        with c4:
            ...
            col_meta = st.selectbox("", labels_meta, index=idx_meta,
                                     key=f"iaf_meta_{ind['id']}", label_visibility='collapsed')
        ...
        with c5:
            ...
            logica = st.selectbox("", logica_labels, index=idx_log,
                                   key=f"iaf_log_{ind['id']}", label_visibility='collapsed')
        ...
        if ind['id'] not in remover_ids:
            indicadores_novos.append({
                **ind,
                ...
                'coluna_real': col_real_val, 'coluna_meta': col_meta_val,
                'logica': logica_val,
            })
    ...
    with a6:
        if st.button("➕ Adicionar", key="iaf_add_btn"):
            if novo_nome.strip():
                ...
                indicadores_novos.append({
                    'id': novo_id, 'nome': novo_nome.strip(), 'pontos': novo_pts,
                    'coluna_real': novo_real_val, 'coluna_meta': novo_meta_val,
                    'tipo': 'automatico', 'logica': novo_log_val,
                })
                st.rerun()
            else:
...
def _render_historico():
    ...
    from modulos.supabase_db import carregar_nps_supabase as _carregar_nps
    ...
    if not historico:
        st.info("Snapshots encontrados mas sem dados suficientes.")
        return

    df_hist = pd.concat(historico, ignore_index=True).sort_values('data_ord')
    ...
    with col2:
        ...
        cons_h = st.multiselect("Filtrar consultores", consultores, key="hist_cons")
    ...
    pivot = df_view.pivot_table(index='consultor', columns='data',
                                 values='classificacao', aggfunc='last')
    datas_cols = sorted(df_view['data'].unique(), key=lambda d: pd.to_datetime(d, dayfirst=True))
...
def render(dados: dict):
    ...
    # CORRIGIDO: usa carregar_nps_supabase
    nps_por_pdv = carregar_nps_supabase()
