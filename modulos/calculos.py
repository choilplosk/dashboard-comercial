...
def resumo_consolidado(base: pd.DataFrame, metas: pd.DataFrame) -> dict:
    # ── Resumo inclui TODOS (gerentes + consultores) ──────────────────────────
    df = base.copy()
    ...
    def meta_soma(col):
        if metas is None or (hasattr(metas, 'empty') and metas.empty): return None
        return pd.to_numeric(metas[col], errors='coerce').sum() if col in metas.columns else None

    def meta_media(col):
        if metas is None or (hasattr(metas, 'empty') and metas.empty): return None
        # Para médias de meta, excluir gerentes para não distorcer a média
